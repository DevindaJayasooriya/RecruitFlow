  ...
  try {
    ...
    const candidates = await new Promise((resolve, reject) => {
      db.all(sql, params, (err, rows) => {
        if (err) {
          return reject(
            new Error("Failed to fetch candidates: " + err.message)
          );
        }
        resolve(rows);
      });
  ...
  try {
    ...
    const candidate = await new Promise((resolve, reject) => {
      db.get(sql, params, (err, row) => {
        if (err) {
          return reject(
            new Error("Failed to fetch candidate by ID: " + err.message)
          );
        }
        resolve(row);
      });
...
const createCandidate = async (candidateData) => {
  try {
    ...
    if (!validReferralStatuses.includes(referralStatus)) {
      throw new Error(
        `Invalid referralStatus: ${referralStatus}. Valid values are: ${validReferralStatuses.join(
          ", "
        )}`
      );
    }
    ...
    if (!validAssessmentStatuses.includes(assessmentStatus)) {
      throw new Error(
        `Invalid assessmentStatus: ${assessmentStatus}. Valid values are: ${validAssessmentStatuses.join(
          ", "
        )}`
      );
    }
    ...
    const result = await new Promise((resolve, reject) => {
      db.run(sql, params, function (err) {
        if (err) {
          return reject(
            new Error("Failed to create candidate: " + err.message)
          );
        }
        resolve(this.lastID);
      });
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      db.run(sql, params, function (err) {
        if (err) {
          return reject(
            new Error("Failed to update candidate: " + err.message)
          );
        }
  ...
  try {
    ...
    const changes = await new Promise((resolve, reject) => {
      db.run(sql, params, function (err) {
        if (err) {
          return reject(
            new Error("Failed to delete candidate: " + err.message)
          );
        }
        if (this.changes === 0) {
