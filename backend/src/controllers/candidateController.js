...
const getCandidateByIdController = async (req, res) => {
  ...
  if (isNaN(id) || id <= 0) {
    return res
      .status(400)
      .json({ error: "Invalid candidate ID: ID must be a positive integer" });
  }
...
const createCandidateController = async (req, res) => {
  if (!req.body || Object.keys(req.body).length === 0) {
    return res
      .status(400)
      .json({ error: "Invalid request body: missing candidate data" });
  }
  try {
    const newCandidateId = await createCandidate(req.body);
    res
      .status(201)
      .json({ id: newCandidateId, message: "Candidate created successfully" });
  } catch (err) {
    ...
    if (
      ...
      err.message.includes("overallScore must be a valid number") ||
      err.message.includes("Invalid referralStatus") ||
      err.message.includes("Invalid assessmentStatus")
    ) {
...
const updateCandidateController = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ changes: changes, message: "Candidate updated successfully" });
  } catch (err) {
...
const deleteCandidateController = async (req, res) => {
  ...
  try {
    const changes = await deleteCandidate(id);
    res
      .status(200)
      .json({
        changes: changes,
